	...
	protected function get_description_with_data( $task_data = [] ) {
		...
		return ( $term && ! \is_wp_error( $term ) )
			? \sprintf(
				...
				$term->name,
				'<a href="' . \esc_url( \progress_planner()->get_ui__branding()->get_url( 'https://prpl.fyi/remove-empty-taxonomy' ) ) . '" target="_blank" data-prpl_accessibility_text="' . \esc_attr__( 'Read more about the removing the empty terms', 'progress-planner' ) . '">' . \esc_html__( 'Read more', 'progress-planner' ) . '</a>'
			)
			: '';
	}
