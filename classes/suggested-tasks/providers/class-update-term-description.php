...
class Update_Term_Description extends Tasks {
	...
	public function get_description_with_data( $task_data = [] ) {
		...
		return $term && ! \is_wp_error( $term ) ? \sprintf(
			...
			$term->name,
			'<a href="' . \apply_filters( 'progress_planner_task_description_link', 'https://prpl.fyi/taxonomy-terms-description', $this->get_task_id(), $this->get_provider_id() ) . '" target="_blank" data-prpl_accessibility_text="' . \esc_attr__( 'Read more about the writing a description for taxonomy terms.', 'progress-planner' ) . '">' . \esc_html__( 'Read more', 'progress-planner' ) . '</a>'
		) : '';
	}
