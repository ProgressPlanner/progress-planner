<?php
/**
 * Class for branding.
 *
 * @package Progress_Planner
 */

namespace Progress_Planner\UI;

/**
 * Class for branding.
 */
final class Branding {

	/**
	 * Default branding post-ID.
	 *
	 * @var int
	 */
	const DEFAULT_BRANDING_POST_ID = 4958;

	/**
	 * Get the api data.
	 *
	 * @return array
	 */
	public static function get_api_data(): array {
		// TODO: Cache the response.
		return self::get_response_from_api();
	}

	/**
	 * Get the response from the API.
	 *
	 * @return array
	 */
	public static function get_response_from_api(): array {
		$response = \wp_remote_get( 'https://progressplanner.com/wp-json/wp/v2/pp-branding/' . self::DEFAULT_BRANDING_POST_ID );
		if ( \is_wp_error( $response ) ) {
			return [];
		}

		return \json_decode( $response['body'], true );
	}

	/**
	 * Print the logo.
	 *
	 * @return void
	 */
	public static function the_logo(): void {
		// Get the logo ID from the API data.
		$logo_id    = self::get_api_data()['acf']['logo'];
		$logo_found = false;
		if ( $logo_id ) {
			// Get the logo URL.
			$response = \wp_remote_get( \progress_planner()->get_remote_server_root_url() . '/wp-json/wp/v2/media/' . $logo_id );
			if ( ! \is_wp_error( $response ) ) {
				$media = \json_decode( \wp_remote_retrieve_body( $response ), true );
				if ( isset( $media['source_url'] ) ) {
					$logo_found = true;
					echo '<img src="' . \esc_url( $media['source_url'] ) . '" alt="Logo" style="height:100px;"/>';
				}
			}
		}

		if ( ! $logo_found ) {
			\progress_planner()->the_asset(
				\progress_planner()->is_pro_site()
					? 'images/logo_progress_planner_pro.svg'
					: 'images/logo_progress_planner.svg'
			);
		}
	}

	/**
	 * Get the color palette.
	 *
	 * @return string
	 */
	public static function get_color_palette(): string {
		$defaults = [
			'--prpl-color-gray-1'             => '#e1e3e7',
			'--prpl-color-gray-2'             => '#d1d5db',
			'--prpl-color-gray-3'             => '#9ca3af',
			'--prpl-color-gray-4'             => '#6b7280',
			'--prpl-color-gray-5'             => '#4b5563',
			'--prpl-color-gray-6'             => '#374151',

			'--prpl-color-accent-red'         => '#f43f5e',
			'--prpl-color-accent-orange'      => '#faa310',
			'--prpl-color-400-orange'         => '#f9b23c',
			'--prpl-color-accent-purple'      => '#0d6b9e',
			'--prpl-color-accent-green'       => '#14b8a6',

			'--prpl-color-headings'           => '#38296d',
			'--prpl-color-text'               => 'var(--prpl-color-gray-5)',
			'--prpl-color-link'               => '#1e40af',

			'--prpl-color-notification-green' => '#16a34a',
			'--prpl-color-notification-red'   => '#e73136',

			'--prpl-background-orange'        => '#fff9f0',
			'--prpl-background-purple'        => '#f6f5fb',
			'--prpl-background-green'         => '#f2faf9',
			'--prpl-background-red'           => '#fff6f7',
			'--prpl-background-blue'          => '#effbfe',
		];

		$api_colors = self::get_api_data()['acf']['colors'];
		$colors     = \wp_parse_args( $api_colors, $defaults );

		$palette_string = '';
		foreach ( $colors as $key => $value ) {
			$palette_string .= $key . ': ' . $value . ';';
		}

		return ":root { $palette_string }";
	}

	/**
	 * Get the admin-menu icon.
	 *
	 * @return string
	 */
	public static function get_admin_menu_icon(): string {
		$icon = 'data:image/svg+xml;base64,';

		$admin_menu_icon_id = self::get_api_data()['acf']['logo'];
		if ( $admin_menu_icon_id ) {
			// Get the logo URL.
			$response = \wp_remote_get( \progress_planner()->get_remote_server_root_url() . '/wp-json/wp/v2/media/' . $admin_menu_icon_id );
			if ( ! \is_wp_error( $response ) ) {
				$media = \json_decode( \wp_remote_retrieve_body( $response ), true );
				if ( isset( $media['source_url'] ) ) {
					// Get the content of the image.
					$content = \wp_remote_get( $media['source_url'] );
					if ( ! \is_wp_error( $content ) ) {
						$content = $content['body'];
						if ( $content ) {
							$icon = 'data:image/svg+xml;base64,' . \base64_encode( $content ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_encode
						}
					}
				}
			}
		}

		return $icon;
	}

	/**
	 * Get the Ravi name.
	 *
	 * @return string
	 */
	public static function get_ravi_name(): string {
		$default  = 'Ravi';
		$api_name = self::get_api_data()['acf']['ravis_name'];

		return $api_name ? $api_name : $default;
	}
}
