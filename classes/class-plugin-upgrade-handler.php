...
class Plugin_Upgrade_Handler {
	 ...
	 * @return array
	 */
	public function get_completed_task_providers() {

		$task_provider_ids = $this->get_added_task_providers();

		$completed_task_providers = [];
		...
		foreach ( $task_provider_ids as $task_provider_id ) {
			...
			if ( $task_provider && method_exists( $task_provider, 'is_task_completed' ) && $task_provider->is_task_completed() ) { // @phpstan-ignore-line
				$completed_task_providers[] = $task_provider;
			}
		}

		return $completed_task_providers;
	}
}
