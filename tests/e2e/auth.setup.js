...
async function globalSetup() {
	...
	try {
		...
		if ( count === 0 ) {
			...
			console.error(
				"3. There is an issue with the plugin's initialization\n"
			);
			await browser.close();
			process.exit( 1 );
		}
		...
		await form
			.locator( 'input[type="submit"].prpl-button-secondary--no-email' )
			.click();
		...
		try {
			...
			await page.waitForSelector( '.prpl-suggested-tasks-list', {
				state: 'visible',
				timeout: 5000,
			} );
