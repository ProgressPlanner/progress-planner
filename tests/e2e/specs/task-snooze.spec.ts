...
test.describe('Task Snooze', () => {
  test('should snooze a task for 1 week', async ({ page, tasksApi }) => {
    ...
    await page.goto('/wp-admin/admin.php?page=progress-planner&prpl_show_all_recommendations');
    ...
    if (!task || task.post_status !== 'publish') {
      console.log(`Task ${snoozeTaskId} not available (status: ${task?.post_status || 'not found'}), skipping`);
      test.skip();
      return;
    }

    await test.step('Snooze the task for 1 week', async () => {
      const taskItem = page.locator(`li[data-task-id="${snoozeTaskId}"]`);
      await expect(taskItem).toBeVisible({ timeout: 10000 });
      await taskItem.hover();

      // Click snooze button
      const snoozeButton = taskItem.locator('button[data-action="snooze"]');
      await snoozeButton.click();

      // Open radio group
      const radioGroup = taskItem.locator('button.prpl-toggle-radio-group');
