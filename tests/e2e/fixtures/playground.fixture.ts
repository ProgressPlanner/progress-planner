...
export const test = base.extend<PlaygroundFixtures, PlaygroundWorkerFixtures>({
  ...
  playgroundServer: [
    async ({}, use, workerInfo) => {
      ...
      console.log(`[Worker ${workerInfo.workerIndex}] Starting Playground on port ${port}...`);
      ...
      await new Promise<void>((resolve, reject) => {
        const timeout = setTimeout(() => {
          reject(new Error('Playground server failed to start within 60s'));
        }, 60000);
        ...
        serverProcess.stdout?.on('data', (data: Buffer) => {
          ...
          if (output.includes('WordPress is running') || output.includes(url)) {
        ...
        serverProcess.stderr?.on('data', (data: Buffer) => {
          console.error(`[Playground Error] ${data.toString()}`);
        });
      ...
      console.log(`[Worker ${workerInfo.workerIndex}] Playground ready at ${url}`);

      await use({ url, process: serverProcess });

      // Cleanup: stop the server
      console.log(`[Worker ${workerInfo.workerIndex}] Stopping Playground...`);
