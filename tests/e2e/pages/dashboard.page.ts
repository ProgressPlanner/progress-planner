...
const SELECTORS = {
  ...
  suggestedTaskCheckbox: '#prpl-suggested-tasks-list .prpl-suggested-task-checkbox:not(:disabled)',

  // Widgets
  widgetWrapper: '.prpl-widget-wrapper.prpl-suggested-tasks',
  suggestedTasksListWidget: '.prpl-widget-wrapper.prpl-suggested-tasks .prpl-suggested-tasks-list',
  ...
  snoozeDurationRadio: '.prpl-snooze-duration-radio-group input[type="radio"]',
...
export class DashboardPage extends BasePage {
  ...
  async goto(options?: { showAllRecommendations?: boolean }): Promise<void> {
  ...
  async createTodo(text: string): Promise<{ taskId: string; element: Locator }> {
    ...
    if (!taskId) throw new Error('Created todo has no task ID');
  ...
  async getTodoText(item: Locator): Promise<string> {
    return await item.locator(SELECTORS.taskItemText).textContent() ?? '';
  }
  ...
  async openCompletedTasks(): Promise<void> {
    ...
    await details.click();
    await this.page.locator(SELECTORS.todoCompletedItem).first().waitFor({
      state: 'visible',
      timeout: 5000,
    }).catch(() => {
  ...
  async getSuggestedTasksCount(): Promise<number> {
    return await this.page.locator(SELECTORS.suggestedTaskCheckbox).count();
  }

  async completeSuggestedTask(): Promise<{ taskId: string | null; previousCount: number }> {
    ...
    const firstCheckbox = this.page.locator(SELECTORS.suggestedTaskCheckbox).first();
  ...
  async snoozeTask(taskId: string, duration: '1-day' | '1-week' | '2-weeks' | '1-month'): Promise<void> {
  ...
  async completeOnboarding(): Promise<void> {
    await expect(this.onboardingPopover).toBeVisible({ timeout: 10000 });

    // Accept privacy policy
    const privacyLabel = this.page.locator(SELECTORS.privacyCheckboxLabel);
    await privacyLabel.click();

    // Start onboarding
    const startButton = this.onboardingPopover.locator(SELECTORS.tourNextButton);
    await startButton.click();

    // Wait for step to advance
    await expect(this.onboardingPopover).toHaveAttribute('data-prpl-step', /^[1-9]/, {
      timeout: 15000,
    });
  ...
  async getTourStepsCount(): Promise<number> {
    return await this.page.evaluate(() => {
      const tour = (window as unknown as { progressPlannerTour?: { steps?: unknown[] } }).progressPlannerTour;
  ...
  async completeTour(): Promise<void> {
    ...
    if (buttonText.toLowerCase() !== 'finish') {
      throw new Error(`Expected "Finish" button, got "${buttonText}"`);
    }
